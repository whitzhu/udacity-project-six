    ...
    Ownable
{
    ...
    modifier paidEnough(uint256 _price) {
        require(
            msg.value >= _price,
            "Error sender insufficient amount to cover the price"
        );
        _;
    }
    ...
    modifier checkValue(uint256 _upc) {
        ...
        address payable payable_address = address(uint160(msg.sender));
        if (amountToReturn > 0) {
            payable_address.transfer(amountToReturn);
        }
    }
    ...
    modifier sold(uint256 _upc) {
        require(items[_upc].itemState == State.Sold, "Error item has not sold");
        _;
    }
        ...
        onlyDistributor
    {
        ...
        address payable payable_address = address(uint160(item.originFarmerID));
        payable_address.transfer(item.productPrice);
        ...
        onlyRetailer
    {
        ...
        Item storage item = items[_upc];
        address payable payable_address = address(uint160(item.distributorID));
        payable_address.transfer(item.productPrice);
        ...
        onlyConsumer
    {
        ...
        Item storage item = items[_upc];
        address payable payable_address = address(uint160(item.retailerID));
        payable_address.transfer(item.productPrice);
        ...
        verifyCaller(items[_upc].retailerID)
    {
        ...
        Item storage item = items[_upc];
        address payable payable_address = address(uint160(item.consumerID));
        payable_address.transfer(item.productPrice);

        // Update the appropriate fields - ownerID, consumerID, itemState
        item.itemState = State.Received;
        item.ownerID = item.retailerID;
        item.consumerID = item.retailerID;
